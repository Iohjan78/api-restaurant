...
import type { PostresCreate } from '@/interfaces/Postres'
...
async function handleSubmit() {  
  ...
  try {
    await postresStore.create(form.value)
    alert('Postre creado correctamente')
    router.push({ name: 'postres_list' })
  } catch (error: any) {
    const mensaje = error.response?.data?.mensaje || 'Error al crear el postre'
    alert(mensaje)
  }
}
