...
import type { BebidasCreate } from '@/interfaces/Bebidas'
...
async function handleSubmit() {
  ...
  try {
    await bebidasStore.create(form.value)
    alert('Bebida creada correctamente')
    router.push({ name: 'bebidas_list' })
  } catch (error: any) {
    const mensaje = error.response?.data?.mensaje || 'Error al crear la bebida'
    alert(mensaje)
  }
}
